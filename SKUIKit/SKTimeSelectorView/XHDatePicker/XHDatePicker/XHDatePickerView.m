...
//        _scrollToDate = [NSDate date];
    ...
    //修改为当前的年份
    NSInteger currentYear = [NSDate dateYesterday].year;
    //可以设置固定年份
    for (NSInteger i = MINYEAR; i < currentYear + 1; i++) {
    ...
    if (!self.maxLimitDate) {
        self.maxLimitDate = [NSDate dateWithTimeIntervalSinceNow:0];
    }
    //最小限制
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    for (int i=0; i<nameArr.count; i++) {
        ...
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(labelX, self.showYearView.frame.size.height/2 - 15/2, 15, 15)];
